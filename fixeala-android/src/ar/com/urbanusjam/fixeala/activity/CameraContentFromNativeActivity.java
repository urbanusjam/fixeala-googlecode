...
public class CameraContentFromNativeActivity extends CameraActivity {
	...
	public void onCreate( final Bundle savedInstanceState ) {
		...
		this.locationListener = new LocationListener() {
			...
			public void onLocationChanged( final Location location ) {
				if (location!=null) {
					CameraContentFromNativeActivity.this.lastKnownLocaton = location;
				if ( CameraContentFromNativeActivity.this.architectView != null ) {
					if ( location.hasAltitude() ) {
						CameraContentFromNativeActivity.this.architectView.setLocation( location.getLatitude(), location.getLongitude(), location.getAltitude(), location.getAccuracy() );
					} else {
						CameraContentFromNativeActivity.this.architectView.setLocation( location.getLatitude(), location.getLongitude(), location.getAccuracy() );
					}
				}
				}
			}
	...
	final Runnable loadData = new Runnable() {
		...
		public void run() {
			...
			CameraContentFromNativeActivity.this.isLoading = true;
			
			final int WAIT_FOR_LOCATION_STEP_MS = 2000;
			
			while (CameraContentFromNativeActivity.this.lastKnownLocaton==null && !CameraContentFromNativeActivity.this.isFinishing()) {
			
				CameraContentFromNativeActivity.this.runOnUiThread(new Runnable() {
					
					@Override
					public void run() {
						Toast.makeText(CameraContentFromNativeActivity.this, R.string.location_fetching, Toast.LENGTH_SHORT).show();	
					}
				});
			...
			if (CameraContentFromNativeActivity.this.lastKnownLocaton!=null && !CameraContentFromNativeActivity.this.isFinishing()) {
				// TODO: you may replace this dummy implementation and instead load POI information e.g. from your database
				CameraContentFromNativeActivity.this.poiData = CameraContentFromNativeActivity.getPoiInformation(CameraContentFromNativeActivity.this.lastKnownLocaton, 20);
				CameraContentFromNativeActivity.this.callJavaScript("World.loadPoisFromJsonData", new String[] { CameraContentFromNativeActivity.this.poiData.toString() });
			}
			
			CameraContentFromNativeActivity.this.isLoading = false;
		}
	};
